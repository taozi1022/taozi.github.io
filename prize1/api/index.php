    ...
    case "zhuli":
        ...
        if(!empty( $row  ) ){
            ...
            if( empty( $rows  ) )
            {
                ...
                /* 增加choujiang_count数值 */
                mysql_query( "UPDATE `choujiang_user` SET `choujiang_count`=`choujiang_count`+0.1 WHERE wxid = '$towxid'  "  );
    ...
    */
    case "choujiang":
        ...
        if(!empty( $row  ) ){
            ...
            $sql = "INSERT INTO `choujiang_jiangpin`(`wxid`,`nickname`,`headimage`, `jiangpin` , `create_date` ) VALUES ('$wxid','$nickname','$headimage','$jiangpin','$create_date')";
            if( mysql_query( $sql ) )
            {
                /*"成功".*/
                //echo mysql_insert_id();
    ...
    /*
           ...
           */
    case "zongbang":
        $sql_bangdan = mysql_query("SELECT *  FROM `choujiang_jiangpin` ORDER BY id desc LIMIT 20 ");
        $results = array();
