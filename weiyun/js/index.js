;(function() {
    ...
    var mask = tools.$("#mask");
    var ok = tools.$("#ok");                  //确定按钮
    var cancle = tools.$("#cancle");          //取消按钮
    var tip = tools.$(".tip")[0];
    var close = tools.$(".close")[0];         //删除时的关闭按钮

    ///////////////////////////////////////////////////////////////////

    //设置内容区与的高度自适应

    var content = tools.$('#content');      //////内容区域
    var header = tools.$("#header");        //////头部
...
/*---------------------自适应高度函数---------------------*/
    function changeSize(){
        ...
        tip.style.left = (clinetW - tip.offsetWidth)/2 + 'px';          //改变提示框的位置
    }
...
/*---------------------提示框---------------------*/
    ...
    function run(obj){
        obj.style.top = -32+'px';
        Movement(obj,{top:0},300,"linear",function(){
            setTimeout(function(){
                Movement(obj,{top:-32},300,"linear")
            },500)
        });
    }
...
/*---------------------新建成功的函数---------------------*/
    ...
    //获取到这个输入框和value
        ...
        if( edtorVal === "" ){                      //新建不成功,移除当前元素
            ...
            //获取到页面中所有的元素
            if(checkboxs.length-1 === 0){           //页面中不存在元素
                disContent.style.display = 'none';  //内容区域隐藏
                file_empty.style.display = 'block'; //空白区域显示
            }
...
tools.addEvent(checkAll,'click',function(){         //给全选按钮添加click事件处理
    if(checkboxs.length === 0 || move.onOff || deleteFile.onOff ) return;             //页面中没有元素的时候，不可以勾选全选框
    ////判断是不是新建
    if(addMkdir.onOff){
        addMkdirFn();
...
tools.addEvent(document,"keydown",function(ev){
    if( addMkdir.onOff && ev.keyCode === 13){  //回车键
        addMkdirFn();     //新建的函数
        empty();    //清空所有class

    }

    if( rename.onOff && ev.keyCode === 13){  //回车键
...
function moveBtnFn(ev){
    ...
    if( select.length === 0 ){      //没有选中文件
        ...
        move.onOff = true;      //移动的状态
        var moveId = 0;         //初始化移动文件的id
        var isMove = true;      //在选择存储位置时使用

        mask.style.display = 'block';

        dialog({
            title:"选择存储位置",
            content:view.moveDialogHtml(),  //内容区域的数据渲染结构
